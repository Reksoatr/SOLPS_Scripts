...
def SOLPSDataSpread(Shot, Attempt):
	...
	for i, item in enumerate(['IonFlx', 'NeuDen', 'Ne', 'Te', 'Ti', 'DN', 'KYE', 'KYI']):
		...
		SOLPSPlotter(str(Shot), Attempt, item,'RadProf', Publish=str(Attempt[0]), SEP=21, LOG10=LG, ax=Spreadfig.add_subplot(gs[COORDR[i],COORDC[i]]))
	...
	for i in Attempt:
		...
		dirT = 'SOLPS_2D_prof/Shot0' + str(Shot) + '/Attempt' + str(i) + '/Output/'
	...
	if N > 1:
       ...
       #plt.savefig('plots/Shot' + Shot + '_Attempts' + '_'.join(Attempt) + '.pdf',dpi=200)
		...
		SOLPSPlotter(str(Shot), Attempt, 'NeuDen', 'Contour', Publish=str(Attempt[0]), SEP=21, LOG10=2, ax=Spreadfig.add_subplot(gs[:,3:]))
